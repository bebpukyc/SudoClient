...
void CMenus::RenderSettings(CUIRect MainView)
{
	...
	if(m_AssetsEditorState.m_VisualsEditorOpen && m_AssetsEditorState.m_FullscreenOpen)
	{
		...
		auto GetRootTabByPage = [&](int Page) {
			...
			if(Page == SETTINGS_TCLIENT || Page == SETTINGS_PROFILES)
		...
		if(ActiveRootTab != ROOT_TAB_BESTCLIENT)
		{
			...
			static CButtonContainer s_aTClientSubTabButtons[2];
		...
		{
			// BestClient root tab - show sub-tabs for steal and bot
			CUIRect SubTabBar;
			ContentView.HSplitTop(24.0f, &SubTabBar, &ContentView);

			const char *apSubTabs[3] = {};
			int aSubTabPages[3] = {};
			int NumSubTabs = 1;
			apSubTabs[0] = Localize("BestClient");
			aSubTabPages[0] = SETTINGS_BESTCLIENT;
			if(!g_Config.m_BcMenuHider)
			{
				apSubTabs[1] = Localize("Bot");
				apSubTabs[2] = "Sudo";
				aSubTabPages[1] = SETTINGS_BOT;
				aSubTabPages[2] = SETTINGS_SUDO;
				NumSubTabs = 3;
			}

			static CButtonContainer s_aBestClientSubTabButtons[3];
			CButtonContainer *pSubButtons = s_aBestClientSubTabButtons;
	...
	if(g_Config.m_UiSettingsPage == SETTINGS_CONFIGS)
		g_Config.m_UiSettingsPage = SETTINGS_TCLIENT;
	if(g_Config.m_BcMenuHider && (g_Config.m_UiSettingsPage == SETTINGS_BOT || g_Config.m_UiSettingsPage == SETTINGS_SUDO))
		g_Config.m_UiSettingsPage = SETTINGS_BESTCLIENT;
	...
	{
		if(i == SETTINGS_LANGUAGE || i == SETTINGS_PLAYER || i == SETTINGS_CONFIGS)
			continue;
		if(g_Config.m_BcMenuHider && (i == SETTINGS_BOT || i == SETTINGS_SUDO))
			continue;
	...
	else if(g_Config.m_UiSettingsPage == SETTINGS_BOT)
	{
		GameClient()->m_MenuBackground.ChangePosition(CMenuBackground::POS_SETTINGS_RESERVED0);
		RenderSettingsBot(MainView);
	}
	else if(g_Config.m_UiSettingsPage == SETTINGS_SUDO)
	{
		GameClient()->m_MenuBackground.ChangePosition(CMenuBackground::POS_SETTINGS_RESERVED0);
		RenderSettingsSteal(MainView);
	}
	else
	{
		dbg_assert_failed("ui_settings_page invalid");
