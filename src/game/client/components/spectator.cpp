...
void CSpectator::OnRender()
{
	...
	for(int i = 0, Count = 0; i < MAX_CLIENTS; ++i)
	{
		...
		const bool ShowBestClientIndicator = g_Config.m_BcClientIndicatorInScoreboard &&
					     ...
					     !GameClient()->m_ClientIndicator.IsPlayerSudoClient(pInfo->m_ClientId);
		...
		// sudo отдельно от Best: совпала версия (своя копия) — подпись sudo
		{
			const int SudoId = GameClient()->m_Snap.m_apInfoByDDTeamName[i]->m_ClientId;
			if(!g_Config.m_BcMenuHider && SudoId >= 0 &&
				GameClient()->m_ClientIndicator.IsPlayerSudoClient(SudoId))
			{
				TextRender()->TextColor(0.75f, 0.3f, 1.0f, PlayerSelected ? 1.0f : 0.5f);
				TextRender()->TextEx(&NameCursor, " [sudo]");
			}
		}
