...
{
	m_pBackgroundLayers = std::make_unique<CLayers>();
	m_pLayers = m_pBackgroundLayers.get();
	m_pBackgroundImages = std::make_unique<CMapImages>();
	m_pImages = m_pBackgroundImages.get();
	m_Loaded = false;
	m_aMapName[0] = '\0';
}
