...
void CScoreboard::RenderScoreboard(CUIRect Scoreboard, int Team, int CountStart, int CountEnd, CScoreboardRenderState &State)
{
	...
	for(int RenderDead = 0; RenderDead < 2; RenderDead++)
	{
		for(int i = 0; i < MAX_CLIENTS; i++)
		{
			...
			// иконка Best — только чужим бестовским; своей копии лепим sudo, не иконку
			const bool SudoCopy = pInfo->m_ClientId >= 0 && GameClient()->m_ClientIndicator.IsPlayerSudoClient(pInfo->m_ClientId);
			if(g_Config.m_BcClientIndicatorInScoreboard && pInfo->m_ClientId >= 0 && ((GameClient()->m_ClientIndicator.IsPlayerBestClient(pInfo->m_ClientId) && (!SudoCopy || pInfo->m_Local)) || pInfo->m_Local))
			{
			...
			// name
			{
				...
				// sudo отдельно от Best: совпала версия (своя копия) — значит SUDO,
				// иконка Best при этом не лепится. В режиме скрытия sudo гаснет.
				if(!g_Config.m_BcMenuHider && pInfo->m_ClientId >= 0 && GameClient()->m_ClientIndicator.IsPlayerSudoClient(pInfo->m_ClientId))
				{
					TextRender()->TextColor(0.75f, 0.3f, 1.0f, TextColor.a);
					TextRender()->TextEx(&Cursor, " [sudo]");
					TextRender()->TextColor(1.0f, 1.0f, 1.0f, TextColor.a);
				}
