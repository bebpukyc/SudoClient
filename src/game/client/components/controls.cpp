...
#include <game/client/components/bestclient/moonwalk.h>
...
#include <game/client/components/aimbot/laserunfreeze.h>
...
int CControls::SnapInput(int *pData)
{
	...
	else
	{
		...
		// Prikoli: moonwalk - when both A and D are held, the resolution above
		// stands still; instead wobble the direction every tick so the tee keeps
		// sliding side to side without any cooldown.
		g_Moonwalk.Apply(
			g_Config.m_AaMoonwalk != 0,
			g_Config.m_ClDummy,
			m_aInputDirectionLeft[g_Config.m_ClDummy] != 0,
			m_aInputDirectionRight[g_Config.m_ClDummy] != 0,
			m_aLastData[g_Config.m_ClDummy].m_Direction,
			&m_aInputData[g_Config.m_ClDummy].m_Direction);
	...
	// Prikoli: aimbot + laser unfreeze (local player only, not the bot).
	// When the laser unfreeze claims the input (it planned a shot or is
	// holding the post-shot turn), the other assists must NOT touch the
	// aim/fire this tick - otherwise the aimbot overwrites the laser
	// direction and the laser fires in a random direction.
	// (aim assist / auto-hammer / auto-aled / hook-spam live in
	// components/aimbot/ now, not in CControls.)
	GameClient()->m_LaserUnfreeze.OnPlayerInput(&m_aInputData[g_Config.m_ClDummy], &m_aLastData[g_Config.m_ClDummy]);
	if(!GameClient()->m_LaserUnfreeze.m_Claimed)
	{
	...
	// steal: full avoid - predicts freeze/death/tele ahead and overrides the
	// input to dodge at the last moment. Runs after the Send check (mirrors the
	// original NWC placement) so the modified dir/jump/hook/aim go to the server
	// with the next send. The silent-aim channel below is applied right after.
	GameClient()->m_Avoid.OnControlsSnapInput(m_aInputData[g_Config.m_ClDummy], Send);

	// steal: silent aim channel (avoid / laser unfreeze) - applied right before
	// hitting the wire and reset for the next tick.
	...
	// BestClient TAS: record/playback/rewind — always runs before the final copy
...
void CControls::OnRender()
{
	...
	// Prikoli: spam emotes from the render loop (network packet, can be sent every frame)
	DoSpamEmote();
}
...
// Prikoli: spam emotes - send a random emoticon every render frame with no
// client-side delay at all. Picks a different emote every time.
void CControls::DoSpamEmote()
{
	if(!g_Config.m_AaSpamEmote)
		return;
	// emotes need an online game with a character to be accepted
