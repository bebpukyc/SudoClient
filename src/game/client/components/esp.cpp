// (c) Kinetix. ESP component — see esp.h for overview. Ported to BestClient
// (immediate mode only — the deferred overlay pass is not ported).
...
static inline unsigned KxEspColor() { return g_Config.m_KxLineEspColor; }
static inline int KxEspSize() { return g_Config.m_KxLineEspSize; }
static inline float KxEspAlpha() { return (float)g_Config.m_KxLineEspAlpha / 100.0f; }
...
void CEsp::EmitLine(const vec2 &p0, const vec2 &p1) const
{
	ColorRGBA col = color_cast<ColorRGBA>(ColorHSLA(KxEspColor(), true));
	col.a = KxEspAlpha();
	int lineSize = KxEspSize();
	...
	if(isDotted)
	{
		...
		if(animOffset < 0.0f)
			animOffset += pattern;
		float pos = animOffset; // +animOffset = dashes move toward target (p1)
		while(pos < totalLen)
		{
			float dashStart = pos;
			...
			if(dashEnd > 0.0f && dashStart < totalLen)
			{
				...
				DrawSegment(segStart, segEnd, col, halfWidth);
			}
			pos += pattern;
		}
		if(hasArrow)
			DrawArrowhead(p0, p1, col, halfWidth);
	}
	else
	{
		DrawSegment(p0, p1, col, halfWidth);
		if(hasArrow)
			DrawArrowhead(p0, p1, col, halfWidth);
	}
}
...
void CEsp::OnRender()
{
	...
	// Set up world projection for immediate-mode drawing.
