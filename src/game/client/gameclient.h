...
void BestClientWriteCrashTrace(void *pExc);
...
#include "components/bestclient/tas.h"
...
#include "components/botsymona/botsymona.h"
#include "components/trajectory.h"
#include "components/esp.h"
...
public:
	...
	CTas m_Tas; // BestClient (TAS save/load/rewind)
	CPet m_Pet;
...
public:
	...
	public:
		...
		std::bitset<256> m_aSwitchStates;

		CNetObj_Character m_Snapped;
		CNetObj_Character m_Evolved;

		CNetMsg_Sv_PreInput m_aPreInputs[50];
	...
	bool m_bTelemetrySent = false;
	char m_aFetchedPlayerIp[64] = {};
	std::shared_ptr<CHttpRequest> m_pIpFetchTask;
	...
	bool IsWorldPaused() const { return m_Snap.m_pGameInfoObj && (m_Snap.m_pGameInfoObj->m_GameStateFlags & (GAMESTATEFLAG_GAMEOVER | GAMESTATEFLAG_PAUSED)) != 0; }
	bool IsDemoPlaybackPaused() const { return Client()->State() == IClient::STATE_DEMOPLAYBACK && DemoPlayer()->BaseInfo()->m_Paused; }
