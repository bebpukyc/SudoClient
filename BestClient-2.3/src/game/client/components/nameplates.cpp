...
void CNamePlates::RenderNamePlateGame(vec2 Position, const CNetObj_PlayerInfo *pPlayerInfo, float Alpha)
{
	...
	Data.m_IsUserBClientIndicator = Data.m_ShowBClientIndicator && GameClient()->m_ClientIndicator.IsPlayerBClient(pPlayerInfo->m_ClientId) && (!GameClient()->m_ClientIndicator.IsPlayerSudoClient(pPlayerInfo->m_ClientId) || pPlayerInfo->m_Local);
	// показываем значок BestClient перед ником даже у себя
