...
void CTrails::OnRender()
{
	...
	for(int ClientId = 0; ClientId < MAX_CLIENTS; ClientId++)
	{
		...
		m_History[ClientId][GameTick % 50] = {
		...
		for(int i = 0; i < TrailLength; i++)
		{
			...
			if(PredictPlayer)
			{
				if(GameClient()->m_aClients[ClientId].m_aPredTick[PosTick % 50] != PosTick)
					continue;
				Part.m_Pos = GameClient()->m_aClients[ClientId].m_aPredPos[PosTick % 50];
				if(i == TrailLength - 1)
					TrailFull = true;
			}
			else
			{
				if(m_History[ClientId][PosTick % 50].m_Tick != PosTick)
					continue;
				Part.m_Pos = m_History[ClientId][PosTick % 50].m_Pos;
