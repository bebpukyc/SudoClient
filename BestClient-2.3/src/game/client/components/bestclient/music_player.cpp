...
bool CMusicPlayer::GetHudThemeColor(ColorRGBA &Out, bool ForcePreview) const
{
	...
	// Играет трек с обложкой — честный базовый цвет обложки (m_Accent), без
	// затемнения в m_Light. Музыки нет — обычный цвет из статика.
	if(m_pImpl && m_pImpl->m_LastArtAnalysis.m_Valid)
		Out = m_pImpl->m_Accent;
	else
		Out = color_cast<ColorRGBA>(ColorHSLA(g_Config.m_BcFunctionsColor));
