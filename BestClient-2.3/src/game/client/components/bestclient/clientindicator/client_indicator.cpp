...
{
	Console()->Register("bc_indicator_status", "", CFGFLAG_CLIENT, ConIndicatorStatus, this, "Show BestClient indicator/presence/sudo detection status");
}

void CClientIndicator::ConIndicatorStatus(IConsole::IResult *pResult, void *pUserData)
{
	(void)pResult;
	static_cast<CClientIndicator *>(pUserData)->PrintStatus();
}

void CClientIndicator::PrintStatus()
{
	char aBuf[512];
	int Peers = 0;
	for(int i = 0; i < MAX_CLIENTS; i++)
		if(m_PresenceCache.IsPresent(i))
			Peers++;
	str_format(aBuf, sizeof(aBuf), "indicator=%d token=%s socket=%s server='%s' peers=%d versions=%d block='%s'",
		g_Config.m_BcClientIndicator,
		EffectiveSharedToken()[0] != '\0' ? "yes" : "no",
		(m_Socket && m_HasServerAddr) ? "yes" : "no",
		m_PresenceCache.ServerAddress().c_str(),
		Peers,
		(int)m_ClientVersions.size(),
		m_LastPresenceBlockReason.empty() ? "none" : m_LastPresenceBlockReason.c_str());
	GameClient()->Echo(aBuf);
	for(const auto &Entry : m_ClientVersions)
	{
		str_format(aBuf, sizeof(aBuf), "  id=%d version='%s'%s", Entry.first, Entry.second.c_str(),
			IsPlayerSudoClient(Entry.first) ? " [SUDO]" : "");
		GameClient()->Echo(aBuf);
	}
}
...
bool CClientIndicator::IsPlayerSudoClient(int ClientId) const
{
	...
	// Своя копия = СТРОГО наш sudo-тег. Базовую BESTCLIENT_VERSION тут не
	// матчим: её анонсируют и чужие паблик-сборки, и sudo лепился левым.
	// Только свежие версии из presence-обмена: мапа по id, чистится при ливе
	// пира, анрегистрации и смене сервера. Браузерные данные (по нику) и
	// "under"-фолбэк не используем — они протухают.
	const auto It = m_ClientVersions.find(ClientId);
	if(It == m_ClientVersions.end() || It->second.empty())
		return false;

	return It->second == SUDOCLIENT_VERSION;
}
...
void CClientIndicator::SendVersionPacket(int ClientId)
{
	...
	const char *pSharedToken = EffectiveSharedToken();
	if(pSharedToken[0] == '\0' || SUDOCLIENT_VERSION[0] == '\0')
		return;
	...
	BestClientIndicator::WriteString(vPacket, PlayerNameForClient(ClientId));
	BestClientIndicator::WriteS16(vPacket, (int16_t)ClientId);
	// анонсируем sudo-тег, а не базовую версию (она у всех Best одинаковая)
	BestClientIndicator::WriteString(vPacket, SUDOCLIENT_VERSION);
	BestClientIndicator::AppendProof(vPacket, pSharedToken);
	...
	DebugLogF("sent %s packet client_id=%d version='%s' player='%s' game_server=%s indicator_server=%s bytes=%d result=%d",
		PacketTypeName(BestClientIndicator::PACKET_VERSION_ANNOUNCE), ClientId, SUDOCLIENT_VERSION, PlayerNameForClient(ClientId), CurrentGameServerAddress(), aServerAddr, (int)vPacket.size(), Sent);
}
...
void CClientIndicator::ProcessIncomingPackets(bool Force)
{
	...
	for(int PacketCount = 0; PacketCount < BestClientIndicator::MAX_RECEIVE_PACKETS_PER_TICK; ++PacketCount)
	{
		...
		// peer_list — полный слепок: версии тех, кого в нём нет, протухли
		for(auto VerIt = m_ClientVersions.begin(); VerIt != m_ClientVersions.end();)
		{
			if(std::find(PeerList.m_vClientIds.begin(), PeerList.m_vClientIds.end(), VerIt->first) == PeerList.m_vClientIds.end())
				VerIt = m_ClientVersions.erase(VerIt);
			else
				++VerIt;
		}
		SchedulePresenceBrowserRefresh();
...
void CClientIndicator::SyncLocalRegistrations(bool Force)
{
	...
	for(auto It = m_RegisteredClientIds.begin(); It != m_RegisteredClientIds.end();)
	{
		if(!DesiredClientIds.Contains(*It))
		{
			...
			// свой id освободили — чужой на этом слоте не должен наследовать версию
