...
bool CCloudInput::TryGetPredPos(const CGameClient &GameClient, int ClientId, int Tick, float Intra, vec2 &OutPos) const
{
	...
	if(GameClient.m_aClients[ClientId].m_aPredTick[(Tick - 1) % 50] != Tick - 1 ||
		GameClient.m_aClients[ClientId].m_aPredTick[Tick % 50] != Tick ||
		GameClient.m_aClients[ClientId].m_aPredTick[Tick % 50] > MaxTick)
		return false;

	OutPos = mix(GameClient.m_aClients[ClientId].m_aPredPos[(Tick - 1) % 50], GameClient.m_aClients[ClientId].m_aPredPos[Tick % 50], Intra);
	return true;
}
