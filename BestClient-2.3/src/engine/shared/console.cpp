...
// BestClient menu hider: наши чит-команды не светятся в таб-комплите и cmdlist,
// пока скрытие включено. Выполняться продолжают (бинды работают).
static bool HideCheatCommandFromList(const char *pName)
{
	if(!g_Config.m_BcMenuHider)
		return false;
	return str_startswith(pName, "aa_") != nullptr ||
		str_startswith(pName, "sym_") != nullptr ||
		str_startswith(pName, "nw_") != nullptr ||
		str_startswith(pName, "kx_") != nullptr;
}
...
int CConsole::PossibleCommands(const char *pStr, int FlagMask, bool Temp, FPossibleCallback pfnCallback, void *pUser)
{
	...
	for(CCommand *pCommand = m_pFirstCommand; pCommand; pCommand = pCommand->Next())
	{
		...
		{
			if(HideCheatCommandFromList(pCommand->m_pName))
				continue;
...
void CConsole::PrintCommandList(EAccessLevel MinAccessLevel, int ExcludeFlagMask)
{
	...
	for(CCommand *pCommand = m_pFirstCommand; pCommand; pCommand = pCommand->Next())
	{
		if((pCommand->m_Flags & m_FlagMask) &&
			...
			!HideCheatCommandFromList(pCommand->m_pName))
		{
